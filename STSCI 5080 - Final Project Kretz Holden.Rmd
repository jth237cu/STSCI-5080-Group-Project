...
```{r diamonds_load}
diamonds <- read.table("diamonds2.txt", header=T)
attach(diamonds)
```
...
summary(supermarket)
hist(price1)
super_plot <- ggplot(supermarket, aes(y=freq, x=price)) + geom_bar(stat="identity", position="dodge") + labs(title = "Supermarket Prelim Analysis", subtitle="for n=10, p=0.95 & λ=9.5")

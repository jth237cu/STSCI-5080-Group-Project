...
t.test(diamonds2$price, mu=4800, conf.level=0.99)
...
```{r diamonds_five_alt}
 ...
 # GIA to IGI
 # HRD to IGI
 cGIA <- diamonds2$X.carat[diamonds2$cert=="GIA"]
 cIGI <- diamonds2$X.carat[diamonds2$cert=="IGI"]
 cHRD <- diamonds2$X.carat[diamonds2$cert=="HRD"]
 mean(cGIA)
 mean(cIGI)
 mean(cHRD)
 ...
 #################################################
 # WILL -- I SAW IN THE SLIDES FOR CHAPTER 12    #
 # SLIDE 16 AND 17                               #
 # "The Multiple Comparisons Problem"            #
 # I think we need to do the tukey               #
 # This matches the boxplot intuitively to me.   #
 #################################################
...
summary(powernap)# THIS SHOULD BE A DIFFERENCE?
t.test(powernap$num2016, powernap$num2015, alternative="less", conf.level=0.95, paired=T) # comment on choice paired=T
...
superdr_plot <- ggplot(super_dt_melt, aes(y=sales, x=promotion, fill=price.reduction)) + geom_boxplot() + labs(title = "Supermarket Prelim Analysis", subtitle="supermarket.txt")
superdr_plot
...
summary(csat_data)
csatbox_plot <- ggplot(csat_data, aes(y=Customer.feedback, x=Role, fill= as.factor(Year))) + geom_boxplot() + labs(title = "Customer Satisfaction Survey", subtitle="N=2617")
csatbox_plot
```
